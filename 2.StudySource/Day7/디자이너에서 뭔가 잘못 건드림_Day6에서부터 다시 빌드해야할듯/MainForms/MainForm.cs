...
namespace MainForms
{
    public partial class MainForm : Form
    {
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            { return; }
            // 메인폼의 조회버튼을 클릭했을 때 실행될 로직 .. 

            // 1번 시나리오 - 품목마스터 객체를 생성해서 조회 메서드를 호출
            //   * 객체가 다르므로 오픈된 화면의 품목마스터 조회 내역을 조회할 수 없다. 
            //ITEMMASTER it = new ITEMMASTER();
            //it.doinquire();

            // 2번 시나리오 - 탭페이지에 등록되어있는 클래스를 가지고와서 실행.
            //   * 모든 화면의 기능을 일일이 하나씩 다 구현해줘야 하기때문에 코드가 매우 길어지고..
            //     새로운 화면이 생길 경우 또 일일이 추가해줘야함.. -> 확장성이 떨어진다. 
            //   * 변경 내역이 존재할 경우 수정해야할 항목들이 늘어난다. -> 유지보수성이 저하된다. 
            //if (myTab.SelectedTab.Name == "ITEMMASTER")
            //{
            //    ITEMMASTER it = (ITEMMASTER)myTab.SelectedTab.Controls[0];
            //    it.DoInquire();
            //}
            //if (myTab.SelectedTab.Name == "UserMaster")
            //{
            //    ITEMMASTER um = (ITEMMASTER)myTab.SelectedTab.Controls[0];
            //    um.DoInquire();
            
            // 3번 시나리오 - 오픈하고자 하는 화면들을 하나의 추상클래스에 연결해서 공통으로 호출해보자.
